...
uninstall_ss() {
    ...
    rm -f "$SERVICE_FILE" "$BIN_FILE" "$CONFIG_FILE" /var/log/shadowsocks-rust.log
    systemctl daemon-reload
    echo "Shadowsocks-Rust 已彻底卸载完成。"
}
...
status_ss() {
    show_status
    journalctl -u shadowsocks-rust -o cat > /var/log/shadowsocks-rust.log
    echo "日志已保存到 /var/log/shadowsocks-rust.log"
}
