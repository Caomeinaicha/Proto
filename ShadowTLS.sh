...
install_shadowtls() {
    ...
    # 获取最新版本（改为 /releases/latest 方式）
    VERSION=$(curl -s https://api.github.com/repos/ihciah/shadow-tls/releases/latest \
        | jq -r '.tag_name')
    ...
    if [ ! -f "$CONFIG_FILE" ]; then
        echo "$(openssl rand -base64 16)" > "$CONFIG_FILE"
    fi
...
uninstall_shadowtls() {
    ...
    rm -f "$SERVICE_FILE" "$BIN_FILE" /root/shadow-tls.log
    systemctl daemon-reload
    echo "Shadow-TLS 已卸载完成"
}
...
status_shadowtls() {
    show_status
    journalctl -u shadow-tls > /root/shadow-tls.log
    echo "日志已保存到 /root/shadow-tls.log"
}
