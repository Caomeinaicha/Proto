...
install_substore() {
    ...
    # 生成 API Key
    if [ ! -f "$SubStoreConfig" ]; then
        echo "$(openssl rand -base64 18 | tr -dc 'A-Za-z0-9' | head -c 20)" > "$SubStoreConfig"
    fi
    ApiKey=$(cat "$SubStoreConfig")
...
uninstall_substore() {
    ...
    rm -rf "$SubStorePath" "$NodePath" /root/substore.log
    systemctl daemon-reload
    echo "Sub-Store 已卸载完成"
}
...
status_substore() {
    show_status
    journalctl -u sub-store.service > /root/substore.log
    echo "日志已保存到 /root/substore.log"
}
