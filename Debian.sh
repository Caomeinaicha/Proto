...
if [ -n "$rawIp6" ]; then
    ...
    [[ $ip6Gate == fe80* ]] && ip6Gate="${ip6Gate}%${interface}"
fi
...
wget --no-check-certificate -qO '/boot/newinstall/initrd.gz' "http://deb.debian.org/debian/dists/bookworm/main/installer-amd64/current/images/netboot/debian-installer/amd64/initrd.gz"
wget --no-check-certificate -qO '/boot/newinstall/vmlinuz' "http://deb.debian.org/debian/dists/bookworm/main/installer-amd64/current/images/netboot/debian-installer/amd64/linux"
...
# 写入 IPv6 配置并补全 DNS
if [ -n "$ip6Addr" ]; then
    # 检查 interfaces 是否已经包含该网卡的 inet6 配置，避免重复
    if ! grep -q "iface \$REAL_IFACE inet6" /etc/network/interfaces; then
...
d-i netcfg/get_ip6nameservers string 2001:4860:4860::8888 2001:4860:4860::8844" )
...
d-i pkgsel/include string openssh-server wget curl net-tools resolvconf
...
#!/bin/sh
exec tail -n +3 \$0
menuentry 'Automatic Install Debian 12 (Dual-Stack Fixed)' {
    insmod part_gpt
    insmod part_msdos
    insmod ext2
...
grub-reboot 'Automatic Install Debian 12 (Dual-Stack Fixed)'
